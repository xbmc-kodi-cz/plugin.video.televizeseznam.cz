...
def list_channels(id, type):
    ...
    for item in items:
        ...
        listing.append(
            (plugin.url_for(list_episodes, item['id'], item['urlName'], 'none',
                            item['category']), listitem, True))
...
def list_episodes(id, urlname, page, category):
    ...
    if (data['data']['tagData']['episodesConnection']['pageInfo']
        ['hasNextPage'] == True):
        listitem = xbmcgui.ListItem(_addon.getLocalizedString(30001))
        listing.append((plugin.url_for(
            list_episodes, id, urlname, data['data']['tagData']
            ['episodesConnection']['pageInfo']['endCursor'],
            category), listitem, True))
...
def get_video(url):
    ...
    if 'hls' in stream_data['pls']:
        ...
        stream_source = stream_data['data']['mp4'][sorted(
            stream_data['data']['mp4'], key=lambda kv: kv[1],
            reverse=False)[0]]['url'][2:]
...
def root():
    ...
    listing.append(
        (plugin.url_for(list_episodes_recent, 'VGFnOjI', 'stream', 'none',
                        'channel_episodes'), listitem, True))

    listitem = xbmcgui.ListItem(
        '[COLOR blue]Stream originals[/COLOR] · {0}'.format(
            _addon.getLocalizedString(30003)))
    listitem.setArt({'icon': 'DefaultTVShows.png'})
    listing.append((plugin.url_for(list_channels, 'VGFnOjI',
                                   'none'), listitem, True))

    listitem = xbmcgui.ListItem(_addon.getLocalizedString(30002))
    listitem.setArt({'icon': 'DefaultRecentlyAddedEpisodes.png'})
    listing.append((plugin.url_for(list_episodes_recent, 'none', 'none',
                                   'none', 'episodes'), listitem, True))
    ...
    listing.append((plugin.url_for(list_episodes_recent, 'VGFnOjEyNDk1OTY',
                                   'nejsledovanejsi-videa-1249596', 'none',
                                   'tag'), listitem, True))

    listitem = xbmcgui.ListItem(_addon.getLocalizedString(30003))
    listitem.setArt({'icon': 'DefaultTVShows.png'})
    listing.append((plugin.url_for(list_channels, 'none',
                                   'none'), listitem, True))
...
def _image(data):
    if data:
        image = list(
            filter(lambda x: x['usage'] == 'poster' or x['usage'] == 'square',
                   data))[0]['url']
